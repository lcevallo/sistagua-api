...
class DireccionClienteListResource(Resource):
    def get(self):
        ...
        if len(keys) == 0:
            ...
            numeros = ['id','fk_cliente','fk_provincia','fk_canton','fk_parroquia']
            ...
            if cedula:
                direcciones_list = self.buscar_x_criterio_x_cedula(cedula,str1.join(column_where))
            else:
    ...
    @classmethod
    def buscar_x_criterio_x_cedula(cls,cedula, criterio_where):
    ...
    def buscar_x_criterio(cls,criterio_where):
        ...
        query = "SELECT  direccion_cliente.* FROM direccion_cliente WHERE direccion_cliente.publish = TRUE {}".format(criterio_where)
        cursor.execute(query)
        rows = cursor.fetchall()
        connection.close()
        data = []
